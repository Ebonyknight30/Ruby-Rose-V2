# Image urls for the psat command
psat_memes = [
    "http://i.imgur.com/5eJ5DbU.jpg",
    "http://i.imgur.com/HBDnWVc.jpg",
    "http://i.imgur.com/RzZlq2j.jpg",
    "http://i.imgur.com/mVRNUIG.jpg",
    "http://i.imgur.com/OvOmC6g.jpg",
    "http://i.imgur.com/QqlSxaZ.png",
    "http://i.imgur.com/finNuzx.jpg",
    "http://i.imgur.com/XB2nBmz.png",
    "http://i.imgur.com/7sCwNXl.jpg",
    "http://i.imgur.com/caw6Pao.png",
    "http://i.imgur.com/GwV0JYL.png"
]

# Response for the 8ball command
magic_conch_shell = [
    "It is certain",
    "It is decidedly so",
    "Without a doubt",
    "Yes definitely",
    "You may rely on it",
    "As I see it yes",
    "Most likely",
    "Outlook good",
    "Yes",
    "Signs point to yes",
    "Reply hazy try again",
    "Ask again later",
    "Better not tell you now",
    "Cannot predict now",
    "Concentrate and ask again",
    "Don't count on it",
    "My reply is no",
    "My sources say no",
    "Outlook not so good",
    "Very doubtful"
]

# Insults for the insult command
insults = [
    "is so insecure about his penis size because it is smaller than a babies",
    "is just a fucking sterotypical 12 year old saying shit like \"I fucked your mom\" and other shit",
    "is a fucking disguisting, disgraceful, ignorant, pathetic, and discriminative weeaboo!",
    "has a kink with 80 year old men",
    "has no other purpose in life other than to be retarded and waste people's time",
    "loves to fap to discord bots",
    "wants the d",
    "has no life",
    "is a furry",
    "is a worthless piece of shit",
    "is an 80 year old man",
]

# Drunk lines for the actdrunk command
drunkaf = [
    "UDNDUNDUNDUNDUDNUDNDUNDUNDUNDUNDUNDUDNUDNDUNDUNDUNDUNDUNDUNDUNDNUDNDUN",
    "AMERICAN IDIOTS YAAAS",
    "HEH HEH HEH HEH IM SO FUKED UP LOL",
    "lol Ill fuk u up n@4f3 fucjing fite me4",
    "fite me u lil fuck",
    "i have somethin to tell you: fedc",
    "weeeeeeew",
    "\*falls*",
    "lol wana fuk some suc tonight #5SdE2@"
]

# Image urls for the honk command
honkhonkfgt = [
    "https://i.imgur.com/c53XQCI.gif",
    "https://i.imgur.com/ObWBP14.png",
    "https://i.imgur.com/RZP2tB4.jpg",
    "https://i.imgur.com/oxQ083P.gif",
    "https://i.imgur.com/byBB7ln.jpg",
    "https://i.imgur.com/NvUiLGG.gif",
    "https://i.imgur.com/QDyvO4x.jpg",
    "https://i.imgur.com/HtrRYSS.png",
    "https://i.imgur.com/bvrFQnX.jpg"
]
jokes = [
    "What's the difference between bullets and humans?\n\nHumans miss John Lennon",
    "Dark humor is like food: not everyone gets it",
    "I got fired from the clock factory after all the extra hours I put in",
    "I used to have a fear of hurdles, but I got over it",
    "I'm not really in the mood for a joke...\nWiess and I just had a HUGE fight...\nshe's been accusing me of acting like a flamingo and I just had to put my foot down",
    "I've been reading a book about antigravity and I just can't put it down",
    "What do you call a fish with no eyes?\n\nA FSH!",
    "Why do seagull fly over the sea?\n\nif the flew over the bay, THEY'D BE BAYGULLS",
    "What's green and has wheels?\n\ngrass. I lied about the wheels",
    "Fisrt rule of thesaurus club: don't talk about, mention, speak of, or chat about thesaurus club",
    "Interviewer: describe yourself in three words\nMe: not good at counting",
    "Interviewer: describe yourself in three words\nMe: lazy",
    "I'm thinking of moving to Sweden; there's so much going for it\n...and the flag is a big plus",
    "Your life",
    "Sixteen Sodium atoms walk into a bar, followed by Batman",
    "A Roman walks into a bar, holds up two fingers and says \"Five beers please\"",
    "Bartender says \"We don't serve time travelers\"\nA time traveler walks into a bar",
    "How many programmers does it take to screw in a light bulb?\n\nnone, that's a hardware problem",
    "What's the difference between an onion and bagpipes?\n\nNo one cries when you cut up a bag pipe",
    "An infinite number of mathmaticians walk into a bar\nFirst mathmatician orders a beer, second orders half a beer, third orders a fourth of a beer\nBartender stops them and pours two beers, saying \"You guys should know your limits\"",
    "How do you get down from an elephant?\n\nYou don't; you get down from a goose",
    "there are 10 types of people in this world: those who know trinary, those who don't, and those who thought this was a binary joke",
    "you mama's so fat when she goes down to the beach whales start singing \"WE ARE FAMILY\nEVEN THOUGH YOU'RE BIGGER THAN ME\"",
    "did you hear about the two antennae that got married?\n\nThe ceremony was okay, but the reception was AMAZING",
    "You can't run in a campground. You can only ran, because it's past tents",
    "I nicknamed my Ipod \"Titanic\"\n\nIt's currently syncing",
    "How many potatoes does it take to kill an Irish man?\n\nZero",
    "I just bought a pair of sneakers from my drug dealer\n\nI don't know what he laced them with, but I've been tripping all day",
    "Pony walks into a bar\nBartender says \"It's karaoke night if you're interested\"\nPony replies \"No, sorry, I'm a little horse",
    "two cats get into a swimming race, one was an american cat named OneTwoThree, and the other was a french cat named UnDeuxTrois\n\nOneTwoThree won becuase the UnDeuxTrois cat sank"
    ]
